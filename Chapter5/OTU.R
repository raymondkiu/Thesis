setwd("U:/Raymond_Kiu/Research/Thesis/Chapter_5/Chicken_16S_data/16S_chicken_OTU/")
...
genusPalette <- c( Acinetobacter="#CDCD00",
                   ...
                   Coprococcus="tan3",
                   Dorea="#003366",
                   Anaerotruncus="#00cc99",
                   Faecalibacterium="#660033",
                   Oscillospira="yellow",
                   Ruminococcus="#66ff33"
)
...
Stack_data <- read.csv("without_probiotics_3_noenterococcus.csv")
#Stack_data_2 <- as.character(Stack_data$ID)
#head(Stack_data_2)
head(Stack_data)

png("OTU-without_biflacenterococcus_nolegend.png",
...
ggplot(df_long[order(df_long$value, decreasing = F),],  aes(x = ID,  y = value,  fill =Species ,drop=TRUE )) +
  ...
  geom_bar(stat ="identity" ,  position = position_fill()) + theme(legend.text = element_text(size=10,face= "italic"),legend.position="none" ,  legend.direction="vertical" , legend.title = element_blank())+
