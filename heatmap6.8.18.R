...
setwd("U:/Raymond_Kiu/Research/Thesis/Chapter_3/Imperial_isolates/Heatmap/")    #set working directory

#########################################################
# reading in data and transform it to matrix format
#########################################################

data <- read.csv("toxin-AMR-cna-ent-plasmid-heatmap-forR-6-8-18.csv")
...
my_palette <- colorRampPalette(c("white","blue4","dodgerblue" ,"steelblue4", "slateblue1","grey","grey"))(n = 699)
col_breaks = c(seq(0,0.7,length=100),          # color 1
               seq(0.8,1.2,length=100),           # color 2
               seq(1.8,2.2,length=100),         # color 3   
               seq(2.8,3.2,length=100),
               seq(3.8,4.2,length=100),
               seq(4.3,4.6,length=100),
               seq(4.7,5.2,length=100))         # color 4     
# seq(3.6,4.0,length=100))         # color 5           
