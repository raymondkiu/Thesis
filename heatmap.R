...
setwd("U:/Raymond_Kiu/Research/Thesis/Chapter_3/Imperial/Heatmap/")    #set working directory

#########################################################
# reading in data and transform it to matrix format
#########################################################

data <- read.csv("toxin-AMR-plasmid.csv")
...
# below is an example of 4 color breaks: uncomment to use the script, change the color as you like,
 my_palette <- colorRampPalette(c("white","blue4","dodgerblue" ,"grey"))(n = 399)
 col_breaks = c(seq(0,0.7,length=100),          # color 1
               seq(0.8,1.2,length=100),           # color 2
               seq(1.4,2.5,length=100),         # color 3   
               seq(2.6,3.1,length=100))         # color 4     
